...
FOLDER_FOR_CODES = Path(settings.BASE_DIR) / "QR-Codes_Test"
...
async def GetPackageInfo(product_id, batch_id, package_id, expiration_date_1, expiration_date_2, status):
    ...
    batch = None
    batch = await Batch.find_one({"id": batch_id})
...
async def TestFindPackage(status):
    db = get_db()
    return await Package.find_one({"status": status})